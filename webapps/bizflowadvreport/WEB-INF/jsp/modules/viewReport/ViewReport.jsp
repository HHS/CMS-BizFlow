...
<%
    String altFlowId = request.getParameter("altFlowId");
    if("viewAdhocReportFlow".equalsIgnoreCase(altFlowId) || "viewReportFlow".equalsIgnoreCase(altFlowId)) {
        if(session.getAttribute("viewIn") == null){
            session.setAttribute("viewIn", "frame");
        }
    }
%>
...
<t:insertTemplate template="/WEB-INF/jsp/templates/page.jsp">
    ...
    <t:putAttribute name="headerContent">
        <script language="JavaScript">
            function openAdHocDataView() {
                var paramResource = "${reportUnitObject.dataSource.referenceURI}";
                var inx = paramResource.lastIndexOf("/");
                var paramParentFolderUri = paramResource.substr(0, inx);
                paramResource = encodeURIComponent(paramResource);
                paramParentFolderUri = encodeURIComponent(paramParentFolderUri);
                window.open("flow.html?_flowId=adhocFlow&ParentFolderUri="+paramParentFolderUri+"&resource=" + paramResource);
                return false;
            }
        </script>
        <style>
        ...
        <t:insertTemplate template="/WEB-INF/jsp/templates/container.jsp">
            ...
            <t:putAttribute name="headerContent">
                ...
                <ul class="list buttonSet">
					<li class="leaf" role="application"> 
					 <button id="dataRefreshButton" type="button" aria-label="<spring:message code="jasper.report.view.button.data.refresh" javaScriptEscape="true"/>" class="button capsule up"><span class="wrap"><span class="icon"></span></span></button></li>
                </ul>
                    ...
                    <ul class="list buttonSet">
                        <c:if test='${param.decorate != "no"}'>
                            <%
                                ...
                                String disabledOpenToEditButton = "disabled='disabled'";
                                String _flowId = request.getParameter("_flowId");
                                if(null != _flowId && _flowId.startsWith("reportTinkerView")) {
                                    showBackButton = false;
                                }
                                ReportUnit reportUnitObject = (ReportUnit) request.getAttribute("reportUnitObject");
                                try {
                                    String refUri = reportUnitObject.getDataSource().getReferenceURI();
                                    if(null != refUri) {
                                        if(refUri.indexOf("/reports/adhoc_reports/") == 0) {
                                            disabledOpenToEditButton = "";
                                        }
                                    }
                                } catch(Exception ex) {
                                    ex.printStackTrace();
                            ...
                            <ul class="list buttonSet">
                                ...
                                <li class="leaf" role="application"><button id="btnOpenAdHocDataView" class="button capsule text up" <%=disabledOpenToEditButton%> onclick="return openAdHocDataView()"><span class="wrap"><spring:message code="openToEditInDesigner"/></span></button></li>
                                <%}%>
                            </ul>
                        </c:if>
                        <li class="node">
                            <ul class="list buttonSet">
                                <c:if test="${isPro}">
                                    <li class="leaf"><button id="fileOptions" class="button capsule mutton up first" aria-label="TAEHO<spring:message code="button.save"/>" disabled="true"><span class="wrap"><span class="icon"></span><span class="indicator"></span></span></button></li>
                                </c:if>
                                <c:if test="${!isPro}">
                                    <li class="leaf"><button id="fileOptions" class="button capsule mutton up first" aria-label="TAEHO2<spring:message code="button.save"/> - <spring:message code="feature.pro.only"/>" disabled="true"><span class="wrap"><span class="icon"></span><span class="indicator"></span></span></button></li>
                                </c:if>
