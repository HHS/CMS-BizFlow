...
setVar(){
	...
	DIR_TOMCAT="/hrts/dev/tomcat8/apache-tomcat-8.0.37"
...
copyBfCustom(){
	...
	then
		...
		cp -rf $SRC_WEBAPPS/../configuration/dev/tomcat/webapps/bizflow $TRG_WEBAPPS
	fi
	echo
}
...
copyWmRuntime(){
	...
	then
		...
		cp -rf $SRC_WEBAPPS/../configuration/dev/tomcat/webapps/bizflowwebmaker $TRG_WEBAPPS
	fi
	echo
}
