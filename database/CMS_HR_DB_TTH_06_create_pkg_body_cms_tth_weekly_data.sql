...
CURSOR CUR_CMS_TTH_DATA 
  ...
  SELECT	ADMIN_CODE || ' - ' || ADMIN_CODE_DESC AS COMPONENT,
      ...
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND PROCESS_COMPLETION_DATE IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(PROCESS_CREATION_DATE, PROCESS_COMPLETION_DATE) <= 5
				THEN 1
				ELSE 0
			END AS ACHIEVED_STRAT_CON,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(PROCESS_CREATION_DATE, PROCESS_COMPLETION_DATE) > 5
				THEN 1
				ELSE 0
      ...
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND ANNOUNCEMENT_OPEN_DATE IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(PROCESS_COMPLETION_DATE ,BIZFLOW.HHS_FN_SUBTRACT_BUSDAY(ANNOUNCEMENT_OPEN_DATE, 1)) <= 12
				THEN 1
				ELSE 0
			END AS ACHIEVED_CLASS,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(PROCESS_COMPLETION_DATE ,BIZFLOW.HHS_FN_SUBTRACT_BUSDAY(ANNOUNCEMENT_OPEN_DATE, 1)) > 12
				THEN 1
				ELSE 0
			END AS MISSED_CLASS,
      ...
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND REVIEW_SENT_DATE IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(ANNOUNCEMENT_CLOSE_DATE,MIN(REVIEW_SENT_DATE)) <= 13
				THEN 1
				ELSE 0
			END AS ACHIEVED_QUALS,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(ANNOUNCEMENT_CLOSE_DATE,MIN(REVIEW_SENT_DATE)) > 13
				THEN 1
				ELSE 0
			END AS MISSED_QUALS,
      ...
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND REVIEW_RETURN_DATE IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(MIN(REVIEW_SENT_DATE), MIN(REVIEW_RETURN_DATE)) <= 11
				THEN 1
				ELSE 0
			END AS ACHIEVED_SELECTION,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(MIN(REVIEW_SENT_DATE), MIN(REVIEW_RETURN_DATE)) > 11
				THEN 1
				ELSE 0
      ...
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND SEND_OFCL_OFFR_CMPL_DATE IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(SEND_TENT_OFFR_CMPL_DATE, SEND_OFCL_OFFR_CMPL_DATE) <= 16
				THEN 1
				ELSE 0
			END AS ACHIEVED_OFCL_OFFER,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(SEND_TENT_OFFR_CMPL_DATE, SEND_OFCL_OFFR_CMPL_DATE) > 16
				THEN 1
				ELSE 0
	...
	LEFT JOIN 
          ...
          CASE WHEN REQUEST_STATUS = 'Request Created' THEN 1 ELSE 0 END AS ACTION_ACTIVE_PRIOR_STRAT_CON,
					HHS_CMS_HR.FN_CONVERT_UTC_TO_LOCAL(PROCESS_CREATION_DATE, 'America/New_York') AS PROCESS_CREATION_DATE,
					HHS_CMS_HR.FN_CONVERT_UTC_TO_LOCAL(PROCESS_COMPLETION_DATE, 'America/New_York') AS PROCESS_COMPLETION_DATE,
					REQUEST_STATUS,
					V_REQUEST_TYPE
		...
		AND HHS_CMS_HR.FN_CONVERT_UTC_TO_LOCAL(PROCESS_CREATION_DATE, 'America/New_York') > '03-FEB-19' 
