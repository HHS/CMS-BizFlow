...
BEGIN
	...
	IF I_PROCID IS NOT NULL AND I_PROCID > 0 THEN
		...
		IF V_XMLVALUE IS NOT NULL THEN
			...
			-- date format and GMT conversion
			-------------------------------------
			-- for cutoff date, append 23:59:00 before GMT conversion
...
IS
	...
	V_XMLDOC                    XMLTYPE;
	V_XMLVALUE                  XMLTYPE;
	REC_MTG_HIST                STRATCON_SCHED_HIST%ROWTYPE;
	TYPE MTG_HIST_TYPE IS REF CURSOR RETURN STRATCON_SCHED_HIST%ROWTYPE;
	CUR_MTG_HIST                MTG_HIST_TYPE;
...
BEGIN
	...
	IF I_PROCID IS NOT NULL AND I_PROCID > 0 THEN
		...
		--
		-- 1. Get Job Request Number from data xml from TBL_FORM_DTL table.
		...
		BEGIN
			...
			(
				SELECT
					...
					--, X.POS_JOB_REQ_NUMBER
					--, X.POS_JOB_REQ_CREATE_DT
					, X.POS_TITLE
				...
				FROM TBL_FORM_DTL FD
					, XMLTABLE('/DOCUMENT/POSITION'
						PASSING FD.FIELD_DATA
						COLUMNS
							--POS_JOB_REQ_NUMBER                  NVARCHAR2(20)   PATH 'POS_JOB_REQ_NUMBER'
							--, POS_JOB_REQ_CREATE_DT             DATE            PATH 'POS_JOB_REQ_CREATE_DT'
							--, POS_TITLE                         NVARCHAR2(100)  PATH 'POS_TITLE'
							POS_TITLE                           NVARCHAR2(100)  PATH 'POS_TITLE'
	...
	--TODO: Form validation doesn't enforce numeric data.  Either validate it by form or check it here.
			...
			--------------------------------
			--DBMS_OUTPUT.PUT_LINE('    STRATCON_SCHED_HIST table');
			MERGE INTO STRATCON_SCHED_HIST TRG
			USING
			(
				SELECT
					V_JOB_REQ_ID AS SSH_REQ_ID
					, X.SSH_ID
				...
				WHERE FD.PROCID = I_PROCID
				ORDER BY X.SSH_ID, SSH_RESCHED_ON
			) SRC ON (SRC.SSH_ID IS NOT NULL AND SRC.SSH_REQ_ID = TRG.SSH_REQ_ID AND SRC.SSH_ID = TRG.SSH_ID)
			WHEN MATCHED THEN UPDATE SET
			...
			-- Update SSH_ID for each record in the form data xml for newly inserted row.
			-- 1. Get STRATCON_SCHED_HIST table records.
			-- 2. For each STRATCON_SCHED_HIST row,
			-- 2.1. Get MEETING_HISTORY record from form data xml that is corresponding to the current STRATCON_SCHED_HIST.
			-- 2.2. If MEETING_HISTORY/record/SSH_ID is not set, assign the value from STRATCON_SCHED_HIST.SSH_ID.
			-- 3. Update form data xml.
			OPEN CUR_MTG_HIST FOR
				SELECT *
				FROM STRATCON_SCHED_HIST
				WHERE SSH_REQ_ID = V_JOB_REQ_ID
				ORDER BY SSH_ID, SSH_RESCHED_ON;
			--DBMS_OUTPUT.PUT_LINE('Opened cursor');

			V_ISMODIFIED := 0;
			V_LOOP_CNT := 0;

			SELECT XMLQUERY('/DOCUMENT/MEETING/MEETING_HISTORY'
				PASSING V_XMLDOC RETURNING CONTENT) INTO V_XMLVALUE
			FROM DUAL;
			--DBMS_OUTPUT.PUT_LINE('V_XMLVALUE = ' || V_XMLVALUE.GETCLOBVAL());

			LOOP
				FETCH CUR_MTG_HIST INTO REC_MTG_HIST;
				EXIT WHEN CUR_MTG_HIST%NOTFOUND;
				V_LOOP_CNT := V_LOOP_CNT + 1;
				V_VALUE := NULL;
				V_SSH_ID := NULL;
				--DBMS_OUTPUT.PUT_LINE('Fetched record, loop count = ' || TO_CHAR(V_LOOP_CNT));

				SELECT XMLQUERY('/MEETING_HISTORY/record[$lc]/SSH_ID/text()'
					PASSING V_XMLVALUE, V_LOOP_CNT AS "lc" RETURNING CONTENT).GETSTRINGVAL() INTO V_VALUE
				FROM DUAL;
				--DBMS_OUTPUT.PUT_LINE('Extract SSH_ID from xml data = ' || V_VALUE);

				-- if current xml record is missing SSH_ID, set it from corresponding record
				IF V_VALUE IS NULL THEN
					V_ISMODIFIED := 1;
					V_SSH_ID := REC_MTG_HIST.SSH_ID;
					SELECT UPDATEXML(V_XMLDOC, '/DOCUMENT/MEETING/MEETING_HISTORY/record[' || TO_CHAR(V_LOOP_CNT) || ']/SSH_ID', XMLTYPE('<SSH_ID>' || V_SSH_ID || '</SSH_ID>'))
					INTO V_XMLDOC
					FROM DUAL;
					--DBMS_OUTPUT.PUT_LINE('V_XMLDOC updated for = ' || TO_CHAR(V_SSH_ID));
					--DBMS_OUTPUT.PUT_LINE('----> V_XMLDOC = ' || V_XMLDOC.GETCLOBVAL());
				END IF;
				--DBMS_OUTPUT.PUT_LINE('V_SSH_ID after = ' || CASE WHEN V_SSH_ID IS NULL THEN 'NULL' ELSE TO_CHAR(V_SSH_ID) END);

			END LOOP;
			--DBMS_OUTPUT.PUT_LINE('V_XMLDOC = ' || V_XMLDOC.GETCLOBVAL());

			-- If xml data is modified, save to form detail table
			IF V_ISMODIFIED = 1 THEN
				UPDATE TBL_FORM_DTL
				SET FIELD_DATA = V_XMLDOC
