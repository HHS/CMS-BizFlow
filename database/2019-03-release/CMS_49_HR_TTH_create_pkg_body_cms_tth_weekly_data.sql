...
CURSOR CUR_CMS_TTH_DATA 
  ...
  SELECT	ADMIN_CODE || ' - ' || ADMIN_CODE_DESC AS COMPONENT,
      ...
      --four fields below are placeholders for Offer Dates
      TRUNC(SYSDATE) AS OFFER_START,
			TRUNC(SYSDATE) AS OFFER_END,
      CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL --AND OFFER_END IS NOT NULL AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(OFFER_START, OFFER_END) <= 15
				THEN 1
				ELSE 0
			END AS ACHIEVED_OFFER,
			CASE
				WHEN REQ_JOB_REQ_NUMBER IS NOT NULL --AND BIZFLOW.HHS_FN_GET_BUSDAYSDIFF(OFFER_START, OFFER_END) > 15
				THEN 1
				ELSE 0
			END AS MISSED_OFFER,
...
SUM_ACHIEVED_OFFER            NUMBER(10);
SUM_MISSED_OFFER              NUMBER(10);
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('WEEKLY RECORD COUNT: ' || CMS_TTH_DATA.COUNT);
         ...
         --Insert record into HHS_CMS_HR.CMS_TIME_TO_HIRE_WEEKLY_PILOT table
          ...
          VALUES 
            ...
            CMS_TTH_DATA(i).OFFER_START,
            CMS_TTH_DATA(i).OFFER_END,
            CMS_TTH_DATA(i).ACHIEVED_OFFER,
            CMS_TTH_DATA(i).MISSED_OFFER);   
        ...
        SUM(MISSED_CLASS), SUM(ACHIEVED_QUALS), SUM(MISSED_QUALS), SUM(ACHIEVED_SELECTION), SUM(MISSED_SELECTION), SUM(ACHIEVED_OFFER), SUM(MISSED_OFFER)
        ...
        SUM_ACHIEVED_QUALS, SUM_MISSED_QUALS, SUM_ACHIEVED_SELECTION, SUM_MISSED_SELECTION, SUM_ACHIEVED_OFFER, SUM_MISSED_OFFER
