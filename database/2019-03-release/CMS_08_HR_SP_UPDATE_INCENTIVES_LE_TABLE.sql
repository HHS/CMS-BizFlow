...
BEGIN
	IF I_PROCID IS NOT NULL AND I_PROCID > 0 THEN 
		...
		IF 0 < l_count THEN
			...
			SELECT count(*) INTO l_count FROM VW_INCENTIVES_LE_CRED WHERE PROC_ID = I_PROCID;
			IF 0 < l_count THEN
				DELETE INCENTIVES_LE_CRED WHERE PROC_ID = I_PROCID AND SEQ_NUM > l_count;
				MERGE INTO INCENTIVES_LE_CRED t
				USING ( SELECT * FROM VW_INCENTIVES_LE_CRED WHERE PROC_ID = I_PROCID ) v
				ON (t.PROC_ID = v.PROC_ID)
				WHEN MATCHED THEN
					UPDATE 
					SET t.SEQ_NUM = v.SEQ_NUM,
						t.START_DATE = v.START_DATE,
						t.END_DATE = v.END_DATE,
						t.WORK_SCHEDULE = v.WORK_SCHEDULE,
						t.POS_TITLE = v.POS_TITLE,
						t.CALCULATED_YEARS = v.CALCULATED_YEARS,
						t.CALCULATED_MONTHS = v.CALCULATED_MONTHS,
						t.CREDITABLE_YEARS = v.CREDITABLE_YEARS,
						t.CREDITABLE_MONTHS = v.CREDITABLE_MONTHS
				WHEN NOT MATCHED THEN
					INSERT (t.PROC_ID,
						t.SEQ_NUM,
						t.START_DATE,
