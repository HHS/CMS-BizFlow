...
CREATE UNIQUE INDEX "HHS_CMS_HR"."ERLR_EMPLOYEE_CASE_PK" 
    ON "HHS_CMS_HR"."ERLR_EMPLOYEE_CASE" 
    (
     "HHSID", "CASE_NUMBER"
    );

DROP VIEW VW_ERLR_EMPLOYEE_CASE;

CREATE VIEW VW_ERLR_EMPLOYEE_CASE
AS SELECT E.CASE_NUMBER, 
    E.HHSID, 
    'F' AS DELETABLE, 
    E.CASE_NUMBER ||'-'|| D.TBL_LABEL||'-'||L.LAST_NAME AS DISPLAY
FROM ERLR_EMPLOYEE_CASE E 
     JOIN HHS_HR.EMPLOYEE_LOOKUP L ON L.HHSID = E.HHSID 
     JOIN ERLR_GEN G ON G.ERLR_CASE_NUMBER = E.CASE_NUMBER 
     JOIN TBL_LOOKUP D ON D.TBL_ID = G.GEN_CASE_TYPE;

CREATE VIEW VW_ERLR_RELATED_CASE
AS SELECT E.CASE_NUMBER, 
