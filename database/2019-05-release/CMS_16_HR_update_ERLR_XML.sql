...
BEGIN
    FOR FORM_REC IN (
              SELECT PROCID, FIELD_DATA, XMLQUERY('/formData/items/item[id="GEN_CASE_CATEGORY"]/value/text()'
               PASSING FIELD_DATA RETURNING CONTENT).getStringVal() as CATEGORY_IDS, MOD_DT
          FROM TBL_FORM_DTL WHERE FORM_TYPE = 'CMSERLR'
    ) 
    LOOP
